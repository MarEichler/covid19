...
```{r import-data-and-join, message=FALSE }
...
plot_gf  <- ggplot(covid19_US_adj, aes(x=Date, y=`Growth Factor`))+
  geom_point()+
  geom_line()+
  geom_hline(yintercept = 1, linetype="dotted", color=AccentCol01)+
  ggtitle("Growth Factor of COVID-19 in the United States by Day")+
  # ADD LABEL 
  geom_label(
      data = current_values
    , aes(label = round(`Growth Factor`, 3))
    )+
  theme_minimal()+
  theme(
     legend.position = "none"
...
Put the plots together and have the plot axes line up by using grobs within the `grid.draw` function.  
```{r, fig.height=7}
...
```{r limit-for-deaths-and-transform}
...
covid19_US_adj %>%
  ...
  row_spec(7, bold=T, color="white", background = "rgba(0, 0, 0, .5)")

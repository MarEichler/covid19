...
ggplot(gf_data, aes(x=date, y= value - 1))+
  annotate("rect", xmin = min(gf_data$date), xmax = max(gf_data$date), ymin = 0, ymax =  Inf, fill = gf2plus, alpha = 0.2) + 
  geom_area(alpha = 0.3, aes(fill = name), show.legend = FALSE) +
  #geom_point(alpha = 0.5, aes(color = name), show.legend = FALSE) +
  scale_fill_manual(name = NULL, values = c("grey30", blue_comp)) + 
  #scale_color_manual(name = NULL, values = c("grey30", blue_comp)) + 
  facet_wrap(
    ~factor(name, labels = c("Daily", "7-Day Moving Average"))
    , nrow = 1
  ) + 
  scale_x_date(
      name = NULL
  ...
  labs(
      title = "Growth Factor Difference from One Last 4 Weeks"
    , subtitle = "A growth factor less than one (difference less than zero) means new cases are decreasing"
  ) +
  theme_minimal()+
  theme(
    plot.title = element_text(hjust = 0.5, size = title_size)
    , plot.subtitle  = element_text(hjust = 0.5, size = subtitle_size - 4)

...
```{r, eval = FALSE }
ggplot(covid19_US_adj, aes(x=date, y= growth_factor - 1))+
  ...
  ggtitle("Daily Growth Factor Difference from One")
```
...
ggplot(covid19_US_adj, aes(x=date, y= MA_growth_factor - 1))+
  ...
  annotate("text", x = x_min + 1, y = -0.5, label = cap_gf, vjust=-0.3, hjust= -0.1, size = 3, color = "grey15") +
  date_scaling +
  scale_y_continuous(expand = c(0, 0), limits = c(-0.5, 0.5), breaks = seq(-0.5, 0.5, .25)) +
  theme_minimal()+
  theme(
    plot.title = element_text(hjust = 0.5, size = subtitle_size)
    , axis.title.x=element_blank()
  ...
  ggtitle("7-Day Moving Average of Growth Factor Difference from One")
```

```{r}
covid19_US_adj %>%
  select(date, contains("growth_factor")) %>%
  pivot_longer(-date) %>%
ggplot(aes(x=date, y= value - 1))+
  annotate("rect", xmin = x_min, xmax = x_max, ymin = 0, ymax =  Inf, fill = gf2plus, alpha = 0.2) + 
  ...
  scale_y_continuous(expand = c(0, 0)) +
  labs(
      title = "Growth Factor Difference from One"

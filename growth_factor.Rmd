...
ggplot(gf_data, aes(x=date))+
  annotate("rect", xmin = min(gf_data$date), xmax = max(gf_data$date), ymin = 0, ymax =  Inf, fill = gf2plus, alpha = 0.15) + 
...
<br>  
```{r gf-natl-diff-one, fig.height = 2.5}
...
ggplot(gf_data4wk, aes(x=date))+
  annotate("rect", xmin = min(gf_data4wk$date)-0.5, xmax = max(gf_data4wk$date)+0.5, ymin = 0, ymax =  Inf, fill = gf2plus, alpha = 0.15) + 
  geom_col(aes(y = gf - 1), alpha=0.3) +
  geom_area(
    aes(y = gf_MA7 - 1)
    , fill = blue_comp
  ...
  scale_y_continuous(minor_breaks = NULL) + 
  labs(
      title = "USA National Daily and 7-Day Moving Average Growth Factor Last 4 Weeks"
    , subtitle =  "Growth Factor Difference from One" 
    , caption = "A growth factor less than one (difference less than zero) means new cases are decreasing"
  ) +
  theme_minimal()+
...
gf_facet_val <- covid19 %>%
  ...
  filter(date > max(date) - 28)  #x_min from parameters.R file 
...
ggplot(gf_facet, aes(date, y = gf_MA7 - 1)) + 
    ...
    labs(
      title = "Individual State 7-Day Moving Average Growth Factor Last 4 Weeks"
      , subtitle =  "Growth Factor Difference from One"
      , caption = paste0(
  ...
  mutate(
    `Growth Factor` = cell_spec(
      `Growth Factor`, color = case_when(
              gf > 1 ~ gf2plus
            , gf > 0 ~ colorspace::lighten(gf2plus, 0.4)
            , TRUE   ~ "grey35"
          )
      , bold = T
      ), 
    `Growth Factor 7-day MA` = cell_spec(
      `Growth Factor 7-day MA`, color = case_when(
              gf_MA7 > 1 ~ gf2plus
            , gf_MA7 > 0 ~ colorspace::lighten(gf2plus, 0.4)
            , TRUE   ~ "grey35"
          )
      , bold = T
      )

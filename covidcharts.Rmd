...
cap_gf <- "A growth factor above 1 means new cases are increasing,\ni.e. the pandemic is growing"
```


```{r}
#plot set up 
plot_gf_setup  <- ggplot(covid19_US_adj, aes(x=date, y=growth_factor))+
  annotate(
      "rect"
    , xmin = x_min
    , xmax = x_max
...
plot_dc_setup <- ggplot(covid19_US_adj, aes(x=date, y=new_cases))+
  scale_x_date(
      labels = date_format("%b-%d")
    , limits = c(x_min, x_max) 
    , expand = c(0, 0)
...
grid.arrange(
  ...
  , nrow = 1
  ,top=textGrob(
      "COVID-19 in the United States by Day"
    , gp=gpar(fontsize=16)
  )
)
```
...
#grid.arrange(gGFsmooth, gDCsmooth, ncol=2)
...
grid.arrange(
  ...
  , nrow = 1
  ,top=textGrob(
      "COVID-19 in the United States by Day, LOESS Smoothing"
    , gp=gpar(fontsize=16)
  )
)
```

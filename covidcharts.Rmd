...
here </a>
and is called `full_data.csv`; also called "All four metrics".  There is an option to download data directly from
...
European CDC </a>
; however Our World in Data has "cleaned up" these data, making them easier to use.  

Given that the data from Our World in Data is updated daily based on London time; the datas and values do not match up with the
<a href="https://www.cdc.gov/coronavirus/2019-ncov/cases-updates/cases-in-us.html" target="_blank">
American CDC </a> 
values.  However, I am sticking with the OWID data set as it is available for download; where the CDC data is not.  
...
plot_td <- ggplot(covid19_US_adj, aes(x=date, y=total_deaths))+
  ...
  geom_label(
    ...
    # position label on bar chart; 85% of the way up 
    , position = position_stack(vjust = 0.85)
    )+
  theme_minimal()+
  theme(

...
$$
\text{Growth Rate} = \frac{ \text{Daily-Cases}_N}{\text{Daily-Cases}_{N-1}}
$$
...
plot_dc <- ggplot(covid19_US_adj, aes(x=Date, y=`Daily Cases`))+
  #use geom_col because counts per day have alread been counted 
  geom_col(alpha=0.6)+
  ggtitle("Daily Cases of COVID-19 in the United States by Day")+
  # ADD LABEL 
  geom_label(
      data = current_values
    #format label so there is a comma after three places 

...
Moving averages calcualted with $k = 5$.  
...
plot_MA_gf  <- ggplot(covid19_US_adj, aes(x=date, y=MA_growth_factor))+
 ...
 # geom_hline(yintercept = 1, linetype="dotted", color=AccentCol01, size=1)+
  ggtitle("Moving Average of Growth Factor of COVID-19 in the United States by Day")+
  theme_minimal()+
  theme(
   #  legend.position = "none"
    ,axis.title.x=element_blank()
...
plot_MA_dc <- ggplot(covid19_US_adj, aes(x=date, y=MA_new_cases))+
  #use geom_col because counts per day have alread been counted 
  geom_line(alpha=0.6, size= 1.1)+
  ggtitle("Moving Average New Cases of COVID-19 in the United States by Day")+
  theme_minimal()+
  theme(
    #remove all legents 
     legend.position = "none"
...
gMAGF <- ggplotGrob(plot_MA_gf)
gMADC <- ggplotGrob(plot_MA_dc)
grid.newpage()
grid.draw(rbind(gMAGF, gMADC))
```

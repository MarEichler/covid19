...
\frac{ \text{New-Cases}_N}{\text{New-Cases}_{N-1}} & \text{if } \text{New-Cases}_{N_1} \neq 0 
\\[1ex]
\text{New-Cases}_N & \text{if } \text{New-Cases}_{N_1} = 0 
\end{cases}
$$
...
plot_gf  <- ggplot(covid19_US_adj, aes(x=date, y=growth_factor))+
  annotate("rect", xmin = x_min, xmax = x_max, ymin = 1, ymax =  Inf, fill = gf2plus, alpha = 0.3) +
  geom_line(alpha=0.3) +
  geom_area(alpha = 0.6) +

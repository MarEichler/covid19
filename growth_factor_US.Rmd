...
```{r packages-global-options, results="hide", warning=FALSE, message=FALSE, include = FALSE, echo = FALSE }
...
What if instead of looking at the growth rate and average of the growth rate - we calculated the growth rate on the average of new cases?  I'm going to call this $\text{GF}_{14}$ to represent that it's the growth rate based on the 14-dayy moving average of new cases.  
...
plot_ma_nc_14 <- ggplot(covid19_US_adj, aes(x=date, y=ma_nc_14))+
#  geom_line(alpha=0.6) +
  geom_area(alpha = 0.3) +
  date_scaling +
  thousands_scaling +
...
Turns out it doesn't make a significant difference!  Even when looking the growth rate based on the 14-day moving average of new cases (smoothing over the cyclical nature of new cases reporting) - results is a growth factor, $\text{GF}_{14}$ that is not deviating from 1.  This leads be to believe that although the trends of new cases appear to be moving downward, we are not in a stable recovery right now.  
...
covid19_US_adj %>%
  ...
  mutate(
    ...
    , `New Cases 14-Day MA` = formatC(ma_nc_14, big.mark=",", format="d")
  ) %>%
  ...
  mutate(
    ...
    `Growth Factor 14-day MA` = cell_spec(
      `Growth Factor 14-day MA`, color = 
          ifelse( MA_growth_factor > 1, gf2plus, colorspace::lighten(gf2plus, 0.4))
      , bold = T
      )
  ) %>%
